...
class TestStorage:
    async def test_list_isos(self, pve_client: ProxmoxClient):
        result = await list_isos(pve_client, storage="local")
        assert isinstance(result, str)

    async def test_get_storage(self, pve_client: ProxmoxClient):
        result = await get_storage(pve_client, storage="local")
        assert isinstance(result, str)

    async def test_storage_status(self, pve_client: ProxmoxClient):
        result = await storage_status(pve_client, node="pve", storage="local")
        assert isinstance(result, str)


class TestDisks:
    async def test_list_disks(self, pve_client: ProxmoxClient):
        result = await list_disks(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_lvm(self, pve_client: ProxmoxClient):
        result = await list_lvm(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_zfs(self, pve_client: ProxmoxClient):
        result = await list_zfs(pve_client, node="pve")
        assert isinstance(result, str)


class TestCapabilities:
    async def test_list_cpu_models(self, pve_client: ProxmoxClient):
        result = await list_cpu_models(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_machine_types(self, pve_client: ProxmoxClient):
        result = await list_machine_types(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_capabilities(self, pve_client: ProxmoxClient):
        result = await list_capabilities(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_capabilities_qemu(self, pve_client: ProxmoxClient):
        result = await list_capabilities_qemu(pve_client, node="pve")
        assert isinstance(result, str)


class TestHardware:
    async def test_list_pci(self, pve_client: ProxmoxClient):
        result = await list_pci(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_usb(self, pve_client: ProxmoxClient):
        result = await list_usb(pve_client, node="pve")
        assert isinstance(result, str)


class TestMapping:
    async def test_list_pci_mappings(self, pve_client: ProxmoxClient):
        result = await list_pci_mappings(pve_client)
        assert isinstance(result, str)

    async def test_list_usb_mappings(self, pve_client: ProxmoxClient):
        result = await list_usb_mappings(pve_client)
        assert isinstance(result, str)

    async def test_list_dir_mappings(self, pve_client: ProxmoxClient):
        result = await list_dir_mappings(pve_client)
        assert isinstance(result, str)


class TestNetworking:
    async def test_list_network(self, pve_client: ProxmoxClient):
