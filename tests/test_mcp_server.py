...
SECRETS_PATH = Path(
    os.environ.get("PROXMOX_SECRETS", "/home/kasm-user/.config/opencode/secrets/proxmox.json")
)
...
def _make_env() -> dict[str, str]:
    ...
    env = dict(os.environ)
    env["PROXMOX_URL"] = secrets.get(
        "PVE_URL", secrets.get("PROXMOX_URL", "https://10.96.16.19:8006")
    )
    env["PROXMOX_MONITOR_TOKEN_ID"] = secrets.get(
        "PVE_MONITOR_TOKEN_ID", secrets.get("PROXMOX_MONITOR_TOKEN_ID", "")
    )
    env["PROXMOX_MONITOR_TOKEN_SECRET"] = secrets.get(
        "PVE_MONITOR_TOKEN_SECRET", secrets.get("PROXMOX_MONITOR_TOKEN_SECRET", "")
    )
    env["PROXMOX_ADMIN_TOKEN_ID"] = secrets.get(
        "PVE_ADMIN_TOKEN_ID", secrets.get("PROXMOX_ADMIN_TOKEN_ID", "")
    )
    ...
    env["PROXMOX_DEFAULT_NODE"] = secrets.get(
        "PVE_NODE", secrets.get("PROXMOX_DEFAULT_NODE", "pve")
    )
    ...
    async def test_lxc_lifecycle_via_mcp(self):
        async with stdio_client(_server_params()) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                create_result = await session.call_tool("proxmox_create_lxc", {
                    "node": "pve",
    ...
    async def test_vm_lifecycle_via_mcp(self):
        async with stdio_client(_server_params()) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                create_result = await session.call_tool("proxmox_create_vm", {
                    "node": "pve",
                    "name": "mcp-test-vm",
                    "cores": 1,
                    "memory": 512,
