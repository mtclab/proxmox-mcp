...
class TestHA:
    async def test_list_ha_resources(self, pve_client: ProxmoxClient):
        result = await list_ha_resources(pve_client)
        assert isinstance(result, str)

    async def test_list_ha_groups(self, pve_client: ProxmoxClient):
        result = await list_ha_groups(pve_client)
        assert isinstance(result, str)

    async def test_ha_status(self, pve_client: ProxmoxClient):
        result = await ha_status(pve_client)
        assert isinstance(result, str)

    async def test_list_ha_rules(self, pve_client: ProxmoxClient):
        result = await list_ha_rules(pve_client)
        assert isinstance(result, str)


class TestBackups:
    async def test_list_backups(self, pve_client: ProxmoxClient):
        result = await list_backups(pve_client, node="pve", storage="local")
        assert isinstance(result, str)


class TestTemplates:
    async def test_list_templates(self, pve_client: ProxmoxClient):
        result = await list_templates(pve_client, node="pve")
        assert isinstance(result, str)

    async def test_list_storage_templates(self, pve_client: ProxmoxClient):
        result = await list_storage_templates(pve_client, node="pve", storage="local")
        assert isinstance(result, str)


class TestMetrics:
    async def test_list_metric_servers(self, pve_client: ProxmoxClient):
        result = await list_metric_servers(pve_client)
        assert isinstance(result, str)

    async def test_metrics_index(self, pve_client: ProxmoxClient):
        result = await metrics_index(pve_client)
        assert isinstance(result, str)


class TestNotifications:
    async def test_list_notification_targets(self, pve_client: ProxmoxClient):
        result = await list_notification_targets(pve_client)
        assert isinstance(result, str)

    async def test_list_notification_matchers(self, pve_client: ProxmoxClient):
        result = await list_notification_matchers(pve_client)
        assert isinstance(result, str)


class TestReplication:
    async def test_list_replication(self, pve_client: ProxmoxClient):
        result = await list_replication(pve_client)
        assert isinstance(result, str)


class TestCeph:
    async def test_ceph_status(self, pve_client: ProxmoxClient):
        try:
    ...
    async def test_ceph_metadata(self, pve_client: ProxmoxClient):
        try:
...
class TestACME:
    async def test_list_acme_accounts(self, pve_client: ProxmoxClient):
        result = await list_acme_accounts(pve_client)
        assert isinstance(result, str)

    async def test_list_acme_plugins(self, pve_client: ProxmoxClient):
        try:
...
class TestCertificates:
    async def test_list_certificates(self, pve_client: ProxmoxClient):
        result = await list_certificates(pve_client, node="pve")
        assert isinstance(result, str)


class TestFirewall:
    async def test_list_cluster_firewall_rules(self, pve_client: ProxmoxClient):
        result = await list_cluster_firewall_rules(pve_client)
        assert isinstance(result, str)

    async def test_list_cluster_firewall_aliases(self, pve_client: ProxmoxClient):
        result = await list_cluster_firewall_aliases(pve_client)
        assert isinstance(result, str)

    async def test_list_cluster_firewall_ipsets(self, pve_client: ProxmoxClient):
        result = await list_cluster_firewall_ipsets(pve_client)
        assert isinstance(result, str)

    async def test_get_cluster_firewall_options(self, pve_client: ProxmoxClient):
        result = await get_cluster_firewall_options(pve_client)
        assert isinstance(result, str)


class TestSDN:
    async def test_list_sdn_zones(self, pve_client: ProxmoxClient):
        result = await list_sdn_zones(pve_client)
        assert isinstance(result, str)

    async def test_list_sdn_vnets(self, pve_client: ProxmoxClient):
        result = await list_sdn_vnets(pve_client)
        assert isinstance(result, str)

    async def test_list_sdn_controllers(self, pve_client: ProxmoxClient):
