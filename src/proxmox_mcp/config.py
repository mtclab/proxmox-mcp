...
class EndpointConfig:
    ...
    def to_config(self, global_verify: str | bool | None = None, global_timeout: int | None = None) -> Config:
        mon_secret = self.monitor_token_secret or os.environ.get(
            f"PROXMOX_{self.name.upper()}_MONITOR_SECRET", ""
        )
        adm_secret = self.admin_token_secret or os.environ.get(
            f"PROXMOX_{self.name.upper()}_ADMIN_SECRET", ""
        )
        tok_id = self.monitor_token_id
        if not tok_id:
            raise ValueError("monitor_token_id is required")
        adm_id = self.admin_token_id
        if not adm_id:
            raise ValueError("admin_token_id is required")
...
class MultiConfig:
    ...
    def validate(self) -> None:
        ...
        for ep in self.endpoints:
            ...
            if not _ENDPOINT_NAME_RE.match(ep.name):
                raise ValueError(
                    f"Endpoint name must match {_ENDPOINT_NAME_RE.pattern!r} — got {ep.name!r}"
                )
    ...
    def from_env(cls, dotenv_path: str | Path | None = None) -> MultiConfig:
        ...
        if not url:
            raise ValueError(
                "No Proxmox configuration found. Set PROXMOX_URL or PROXMOX_ENDPOINTS_JSON."
            )
