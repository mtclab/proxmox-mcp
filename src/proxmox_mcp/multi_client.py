...
class MultiClient:
    ...
    def __init__(self, config: MultiConfig) -> None:
        ...
        for ep in config.endpoints:
            single_config = ep.to_config(
                global_verify=config.verify, global_timeout=config.timeout
            )
                    ...
                    for node_info in self._nodes_cache[name]:
                        ...
                        if node_name:
                            ...
                            if existing is None:
                                ...
                                existing != name
                                and name == self.default_endpoint
                            ):
                                self._node_to_endpoint[node_name] = name
    ...
    def healthy_endpoints(self) -> list[str]:
        return [n for n, ok in self._healthy.items() if ok] or [
            self.default_endpoint
        ]

    def get_client(
        self, elevated: bool = False, endpoint: str | None = None
    ) -> Any:
    ...
    async def resolve_node(
        self, node: str | None = None, endpoint: str | None = None
    ) -> ResolvedNode:
    ...
    async def resolve_guest(
        ...
        return ResolvedGuest(
            endpoint=resolved.endpoint, node=resolved.node, vmid=vmid
        )
    ...
    async def safe_api_call(
        ...
        **kwargs,
        ...
        ep = endpoint or self.default_endpoint
        return await self.clients[ep].safe_api_call(
            func, *args, elevated=elevated, **kwargs
        )

    async def cluster_call(
        self, api_call_func, *args, elevated: bool = False, **kwargs
    ) -> Any:
        tried = []
        for ep in [self.default_endpoint] + [
            e for e in self.healthy_endpoints if e != self.default_endpoint
        ]:
            ...
            try:
                return await self.clients[ep].safe_api_call(
                    api_call_func, *args, elevated=elevated, **kwargs
                )
            except ProxmoxConnectionError:
        ...
        raise ProxmoxConnectionError(
            f"All endpoints unreachable (tried: {tried})"
        )
    ...
    def upload(
        ...
        ep = endpoint or self.default_endpoint
        return self.clients[ep].upload(
            node, storage, content_type, file_obj, filename=filename, elevated=elevated
        )
